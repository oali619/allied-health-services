          ...
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 text-base font-semibold leading-7 text-gray-600 sm:grid-cols-2 md:flex lg:gap-x-10">
            {hssServiceCategories.map((service, index) => (
              <div key={service.name}>
                {service.name} {index !== hssServiceCategories.length - 1 && <span aria-hidden="true">&rarr;</span>}
              </div>
            ))}
          </div>
          <div className="mt-16 grid grid-cols-1 gap-8 sm:mt-20 sm:grid-cols-4">
              ...
              <div key={hssServiceType.name}>
                  <Disclosure as="div" className='pb-6'>
                    <DisclosureButton className="group flex items-center justify-between gap-2 font-semibold text-gray-700">
                      ...
                      {disclosureIsOpen
                        ?
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6" onClick={() => setDisclosureIsOpen(!disclosureIsOpen)}>
                    ...
                    <DisclosurePanel className='tracking-tight text-gray-500'>{hssServiceType.value}</DisclosurePanel>
                  </Disclosure>
              </div>
            ))}
          </div>
